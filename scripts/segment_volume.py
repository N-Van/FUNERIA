...
def main():
    ...
    for z in range(Z):
        ...
        masks_f, info = model.infer_one_projection(
            frame,
            urna_mask=urna_mask_np,
            mode="auto",
            # grid_stride=stride,
            points_batch_size=bsz)  # (N,H,W) bool
        ...
        if (z % 25) == 0:
            print(f"z={z}/{Z}  raw={info['n_raw']} keep={info['n_keep']} points={info['n_points']}")
